...
<section class="content-header">
    ...
    <ol class="breadcrumb">
        ...
        <li class="active">Kho Map</li>
    </ol>
</section>
<section class="content">
    <?php if ($this->admin->type == $this->adminRoot->type) : ?>
        <div class="mb-1">
            <button class="btn btn-success" data-toggle="modal" data-target="#warehouse-map-import" data-backdrop="static" data-keyboard="false"><i class="fa fa-plus"></i> Nhập kho</button>
        </div>
    <?php elseif (in_array('action_import', $permissions->warehousemap, true)) : ?>
        <div class="mb-1">
            <button class="btn btn-success" data-toggle="modal" data-target="#warehouse-map-import" data-backdrop="static" data-keyboard="false"><i class="fa fa-plus"></i> Nhập kho</button>
        </div>
    <?php endif; ?>
    <div class="box">
        ...
        <div class="box-body table-responsive no-padding mailbox-messages">
            <table class="table table-hover" id="table-search">
                <form method="GET" action="<?= base_url('warehousemap') ?>">
                    <tr>
                        <th style="width: 30%">
                        ...
                        <th style="width: 20%">
                            <div class="form-group no-margin">
                                <select name="color" class="form-control select2 color">
                                    <option value="none">-- Chọn màu --</option>
                                    <?php if (!empty($colors)) : ?>
                                        <option value="required" <?= $this->input->get('color') == 'required' ? 'selected' : '' ?>>Theo yêu cầu</option>
                                        <?php foreach ($colors as $color) : ?>
                                            <option value="<?= $color ?>" <?= $this->input->get('color') == $color ? 'selected' : '' ?>><?= $color ?></option>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
                                </select>
                            </div>
                        ...
                        <th style="width: 20%">
                            <div class="form-group no-margin">
                                <select name="size_id" class="form-control select2 size_id">
                                    <option value="none">-- Chọn size --</option>
                                    <?php if (!empty($sizes)) : ?>
                                        <option value="required" <?= $this->input->get('size_id') == 'required' ? 'selected' : '' ?>>Theo yêu cầu</option>
                                        <?php foreach ($sizes as $size) : ?>
                                            <option value="<?= $size->id ?>" <?= $this->input->get('size_id') == $size->id ? 'selected' : '' ?>><?= $size->name ?></option>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
                                </select>
                            </div>
                        ...
                        <th class="pull-right">
                            ...
                            <a href="<?= base_url('warehousemap') ?>" title="Làm mới" class="btn btn-default" type="submit"><i class="fa fa-refresh"></i></a>
                        </th>
                    </tr>
                </form>
            </table>
                ...
                <?php if (!empty($list)) : ?>
                    <?php foreach ($list as $row) : ?>
                        <tr>
                            ...
                            <td><?= $row->color  ?></td>
                            <td><?= $row->size_name ? $row->size_name : $row->size  ?></td>
                            <td><?= $row->kho_name  ?></td>
                            <td><b><?= $row->total  ?></b></td>
                            <td class="text-center">
                                ...
                                <?php elseif (in_array('action_transfer', $permissions->warehousemap, true)) : ?>
                                    <button onclick="loadwarehouse(<?= $row->id ?>, <?= $row->kho_id ?>)" class="btn btn-sm btn-social-icon btn-warning" title="Chuyển kho"><i class="fa fa-retweet fa-fw"></i></button>
                                <?php else : ?>
                                    ---
                                <?php endif; ?>
...
<!-- Nhập kho -->
<?php $this->load->view('warehousemap/import'); ?>
<!-- Chuyển kho -->
<div class="modal fade" id="warehouse-map-transfer">
...
<script>
    $('#table-search .product_id').on("select2:select", function(e) {
        var product_id = $(this).val();
        var parent = $('#table-search');
        $.ajax({
            type: 'post',
            dataType: 'json',
    ...
    function loadwarehouse(warehouseMapId, warehouseFromId) {
        var parent = $('#warehouse-map-transfer .modal-content');
        $.ajax({
            type: "post",
            dataType: 'json',
            url: "<?= base_url('warehousemap/warehouse-to') ?>",
            data: {
            ...
            success: function(result) {
                if (result.status === 1) {
                    ...
                    parent.find('.btn-transfer').attr('data-id', warehouseMapId);
                    $('#warehouse-map-transfer').modal('show');
                } else {
                    console.log(result);
                }
            }
    ...
    $('.btn-transfer').click(function() {
        ...
        var warehouse_map_id = $(this).attr('data-id');
        var warehouse_to = parent.find('.warehouse_to').val();
        var qty = parent.find('.qty').val();
        $.ajax({
            type: "post",
            dataType: 'json',
            url: "<?= base_url('warehousemap/action-transfer') ?>",
            data: {
    ...
    $("#warehouse-map-transfer").on("hidden.bs.modal", function() {
        var parent = $('#warehouse-map-transfer .modal-content');
        parent.find('.warehouse_to').val('');
        parent.find('.qty').val('');
        parent.find('.notification').html('');
